<x-admin-layout>
    <x-slot name="pagescript">
        <script>
            ...
            if($("#has_attributes").is(':checked')) {
                $("#attribute_subsection").removeClass("hidden");
            } else {
                $("#attribute_subsection").addClass("hidden");
            }
            // Open Attributes
    ...
    <x-dashview>  
        <x-slot name="heading">
            Add Attributes For Product - {{$product->name}}
        </x-slot>
        ...
        <x-slot name="breadcrumb">
            ...
            <span> / Specification (or) Attributes </span>
        </x-slot>
        ...
        <div class="w-full h-screen overflow-y-scroll md:flex">
            <div class = "w-full min-h-screen md:w-8/12">
                ...
                <div class="mb-6 border border-gray-200 shadow-md p-3 bg-gray-200">
                    ...
                    <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                        <input type="checkbox" name="has_attributes" id="has_attributes" class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none" {{old('has_attributes') || $product->attributes->count() > 0 == 1 ? "checked" : null}} />
                    ...
                    <div id = "attribute_subsection" class = "hidden px-4 pb-3 py-9">
                        <table class="bg-white shadow-md mb-6">
                            @foreach ($attributes as $attribute)
                                <tr class="p-4">
                                    <td class="w-1/3 p-2"> {{$attribute->title}} </td>
                                    <td class="w-1/3 p-2"> {{$attribute->value}} </td>
                                    <td class="w-1/3">
                                        <form action="/products/{{$product->id}}/attributes/{{$attribute->id}}" method="POST">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <form action="/products/{{$product->id}}/attributes" method="POST">
                            @csrf
                            <label class="block" for=""> Add Attributes </label>
                            <div class="flex flex-none">
                                <div class="w-full md:w-1/2">
                                    <input type="text" name="title" placeholder = "Add Spec Title" class="inline-block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" />
                                    @if($errors->has('title'))
                                        <div class="bg-red-200 text-red-700 p-3 mb-6">
                                            <ul>
                                ...
                                <div class="w-full md:w-1/2">
                                    <input type="text" name="value" placeholder = "Add Spec Description" class="inline-block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" />
                                    @if($errors->has('value'))
                                        <div class="bg-red-200 text-red-700 p-3 mb-6">
                                            <ul>
                            ...
                            <div class="flex justify-between pt-6">
                                <div class="flex justify-between">
                                    <a href = "/products/{{$product->id}}/banners" class = "px-4 py-2 bg-gray-500 text-white hover:bg-gray-600 mr-3"> Go Back To Previous Step </a>
                                    <input type="submit" name = "submit" class = "px-4 py-2 cursor-pointer text-gray-700 bg-amber-300 hover:bg-amber-400" value="Save & Add One More Attribute">
                                </div>
            ...
            <div class = "w-full pt-28 md:w-4/12 sticky top-0"> 
                <x-sidebar_index pid="{{$product->id}}" step="6" newproduct="{{false}}"></x-sidebar_index>
            </div>
        </div>
    </x-dashview>
</x-admin-layout>